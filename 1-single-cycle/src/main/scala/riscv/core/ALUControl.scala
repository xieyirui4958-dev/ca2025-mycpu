    ...
    is(InstructionTypes.OpImm) {
      ...
      io.alu_funct := MuxLookup(io.funct3, ALUFunctions.zero)(
        Seq(
          ...
          InstructionsTypeI.sri   -> Mux(io.funct7(5),
                                        ALUFunctions.sra,  // 1 → sra
                                        ALUFunctions.srl)  // 0 → srl
        )
      )
    }
    ...
    is(InstructionTypes.Op) {
      ...
      io.alu_funct := MuxLookup(io.funct3, ALUFunctions.zero)(
        Seq(
          ...
          //   funct7[5] = 1 → SUB
          // TODO: Complete Mux selection logic
          InstructionsTypeR.add_sub -> Mux(io.funct7(5),
                                           ALUFunctions.sub, // 1 → sub
                                           ALUFunctions.add),// 0 → add
          ...
          // TODO: Complete Mux selection logic
          InstructionsTypeR.sr      -> Mux(io.funct7(5),
                                           ALUFunctions.sra, // 1 → sra
                                           ALUFunctions.srl) // 0 → srl
        )
      )
