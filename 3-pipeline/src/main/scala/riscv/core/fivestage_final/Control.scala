  ...
  when(
    ...
    // c) Load-branch: LW x1, 0(x2) [EX]; BEQ x1, x4, label [ID] → stall
      ...
      || // OR
        ...
        //
      (
      io.jump_instruction_id &&                                  // Jump in ID
      io.memory_read_enable_mem &&                               // Load in MEM
      (io.rd_mem =/= 0.U) &&                                     // rd_mem != x0
      ((io.rd_mem === io.rs1_id) || (io.rd_mem === io.rs2_id))   // MEM destination matches source
    ...
    io.id_flush := true.B     // Insert bubble into ID/EX
    io.pc_stall := true.B     // Stop PC from updating
    io.if_stall := true.B     // Hold IF/ID content
